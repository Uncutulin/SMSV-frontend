...
import { fetchJerarquiaLogs } from '@/services/qstomService';
...
export default function JerarquiaLogsPage() {
    ...
    const loadLogs = async () => {
        setLoading(true);
        try {
            const data = await fetchJerarquiaLogs();
            setLogs(data);
        } catch (error) {
            console.error("Error loading Jerarquia logs:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <DashboardLayout>
            <div className="p-6">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-900 font-outfit">Logs de Jerarquía</h1>
                    <p className="text-gray-600 mt-2">Seguimiento de importación y procesamiento de jerarquías BI.</p>
                </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-100">
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center text-gray-400">
                                            No se encontraron registros de logs de jerarquía.
                                        </td>
                                    </tr>
                                ) : (
                                    logs.map((log) => (
